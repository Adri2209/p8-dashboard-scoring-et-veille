...
def find_similar_clients(train_df, client_row, cols, k=10):
    ...
    client_scaled = scaler.transform(
        client_row[cols].values.reshape(1, -1)
    )
...
client_id = st.sidebar.selectbox(
    "Choisir l'identifiant client",
    options=train_df.index.tolist()
)

if st.sidebar.button("Charger le client"):

    # --------------------------------------------------------
    # Récupération du client et prédiction
    # --------------------------------------------------------
    client_info = api_get(f"/client_info/{client_id}")
    prediction = api_post("/prediction", {"client_id": client_id})

    if not client_info or not prediction:
        st.stop()
    ...
    if simple_mode:
        # Vue simplifiée : on affiche seulement quelques variables clés si disponibles
        cols_to_show = [
            col for col in [
                "NAME_CONTRACT_TYPE",
                "CODE_GENDER",
    ...
    confidence_score = min(100, int(confidence * 200))

    st.metric("Niveau de confiance", f"{confidence_score} / 100")
    ...
    if top_vars:
        st.markdown(
            f"Les variables qui ont le plus influencé la décision du modèle sont : "
            f"**{', '.join(top_vars)}**."
        )
    else:
    ...
    # --------------------------------------------------------
    # 6. Comparaison population globale
    # --------------------------------------------------------
    st.subheader("📈 Comparaison avec la population globale")
    
    if "feature" not in st.session_state:
        st.session_state.feature = None
    st.session_state.feature = st.selectbox("Variable à comparer", usable_columns)

    fig_hist = px.histogram(train_df, x=st.session_state.feature, title=f"Distribution de {st.session_state.feature}")
    if st.session_state.feature in client_series.index:
        fig_hist.add_vline(
            x=client_series[st.session_state.feature],
            line_color="red",
            line_dash="dash",
    ...
    if st.session_state.feature in train_df.columns and st.session_state.feature in client_series.index:

        value = client_series[st.session_state.feature]

        percentile = (train_df[st.session_state.feature] < value).mean() * 100
    ...
    # === PERSISTANCE DES SÉLECTEURS ===
    if "bivar_x" not in st.session_state:
        st.session_state.bivar_x = None

    if "bivar_y" not in st.session_state:
        st.session_state.bivar_y = None

    # === SELECTBOX X
    st.session_state.bivar_x = st.selectbox(
        "Variable X",
        usable_columns,
        index=usable_columns.index(st.session_state.bivar_x) if st.session_state.bivar_x in usable_columns else 0,
        key="bivar_x_fix"
    )

    # === SELECTBOX Y
    usable_y = [v for v in usable_columns if v != st.session_state.bivar_x]
    if st.session_state.bivar_y not in usable_y:
        st.session_state.bivar_y = usable_y[0]

    st.session_state.bivar_y = st.selectbox(
        "Variable Y",
        usable_y,
        index=usable_y.index(st.session_state.bivar_y) if st.session_state.bivar_y in usable_y else 0,
        key="bivar_y_fix"
    )

    var_x = st.session_state.bivar_x
    var_y = st.session_state.bivar_y
    ...
    # === DATA CLEAN ===
    df = train_df[[var_x, var_y, "TARGET"]].dropna().copy()
    df["Risque"] = df["TARGET"].map({0: "Bon payeur", 1: "Défaut"})
    ...
    z_x = zscore(x_val, df[var_x])
    z_y = zscore(y_val, df[var_y])
    ...
    zone = df[
        (df[var_x].between(x_val - dx, x_val + dx)) &
        (df[var_y].between(y_val - dy, y_val + dy))
    ]
    ...
    sens = "évoluent ensemble" if corr > 0 else "évoluent inversement" if corr < 0 else "ne sont pas liées"
    ...
    variables = st.multiselect(
        ...
        usable_columns,
        default=[v for v in ["PAYMENT_RATE", "EXT_SOURCE_2", "INCOME_CREDIT_PERC"] if v in usable_columns]
    )
