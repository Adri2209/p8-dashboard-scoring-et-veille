...
if st.sidebar.button("Charger le client"):
    ...
    feature = st.selectbox("Variable à comparer", usable_columns)
    ...
    if "bivar_x" not in st.session_state:
        st.session_state.bivar_x = usable_columns[0]

    if "bivar_y" not in st.session_state:
        st.session_state.bivar_y = usable_columns[1]

    # === SELECTBOX X
    st.session_state.bivar_x = st.selectbox(
        "Variable X",
        usable_columns,
        index=usable_columns.index(st.session_state.bivar_x),
        key="bivar_x_fix"
    )
    ...
    st.session_state.bivar_y = st.selectbox(
        "Variable Y",
        usable_y,
        index=usable_y.index(st.session_state.bivar_y),
        key="bivar_y_fix"
    )
